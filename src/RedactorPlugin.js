...
(function ($) {
  RedactorPlugins.drawer = function () {
    ...
    var createEasel = function(redactorInstance, width, height){
      ...
      var newEasel = new Easel.Easel(redactorInstance, drawerOptions,
        width, height
      );

      newEasel.on(newEasel.EVENT_CANVAS_MODIFIED, function () {
        for (var i = 0; i < canvases.length; i++) {
    ...
    return {
      init: function () {
        ...
        this.opts.syncCallback = function (html) {
          ...
          // but we still have Easel.  So here we iterate though
          ...
          // without Easel attached to them
      ...
      show: function () {
        this.modal.load('drawer', this.drawer.t('Insert Easel'), 260);
      ...
      insert: function () {
        ...
        var canvasElem = createEasel(this, width, height);
